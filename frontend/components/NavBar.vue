  ...
  <header
    ...
    <div class="max-w-7xl mx-auto flex justify-between items-center px-4 sm:px-6 h-full">
      ...
      <nav
        ...
        <NuxtLink v-for="section in sections" :key="section.id" :to="section.to"
          class="relative px-4 py-2 whitespace-nowrap transition-colors duration-300 outline-none group"
          :class="activeSection === section.id ? 'text-brand-700' : 'text-gray-500 hover:text-gray-900'">
          ...
          <span
            ...
            :class="activeSection === section.id ? 'w-[80%] bg-brand-600 opacity-100' : 'w-0 bg-gray-300 opacity-0 group-hover:w-[40%] group-hover:opacity-100'"></span>
        </NuxtLink>

        <NuxtLink to="/info"
          class="relative px-4 py-2 whitespace-nowrap transition-colors duration-300 outline-none group"
          :class="activeSection === 'info' ? 'text-brand-700' : 'text-gray-500 hover:text-gray-900'">
          <span class="relative z-10">INFORMAÇÕES</span>
          <span
            class="absolute bottom-0 left-1/2 -translate-x-1/2 h-[4px] rounded-t-lg transition-all duration-500 ease-out"
            :class="activeSection === 'info' ? 'w-[80%] bg-brand-600 opacity-100' : 'w-0 bg-gray-300 opacity-0 group-hover:w-[40%] group-hover:opacity-100'"></span>
        </NuxtLink>

        <NuxtLink to="/contato"
          class="relative px-4 py-2 whitespace-nowrap transition-colors duration-300 outline-none group"
          :class="activeSection === 'contato' ? 'text-brand-700' : 'text-gray-500 hover:text-gray-900'">
          <span class="relative z-10">CONTATO</span>
          <span
            class="absolute bottom-0 left-1/2 -translate-x-1/2 h-[4px] rounded-t-lg transition-all duration-500 ease-out"
            :class="activeSection === 'contato' ? 'w-[80%] bg-brand-600 opacity-100' : 'w-0 bg-gray-300 opacity-0 group-hover:w-[40%] group-hover:opacity-100'"></span>
        </NuxtLink>
      ...
      <div title="Redes Sociais" class="hidden md:flex items-center justify-end gap-2 pr-2">
        <a href="https://www.instagram.com/sodaescorpiao/" target="_blank" rel="noopener noreferrer"
          ...
          class="p-2.5 rounded-full text-brand-700 hover:bg-brand-50 hover:text-brand-600 transition-all duration-300 group active:scale-90">
      ...
      <div v-if="menuOpen" id="mobile-navigation" ref="mobileMenu"
        ...
        <div class="p-6 flex flex-col gap-2 relative">
          ...
          <NuxtLink to="/produtos"
            class="relative z-10 block py-3 px-6 font-semibold rounded-2xl transition-all duration-300 active:scale-[0.98] hover:translate-x-1 origin-left"
            :class="activeSection === 'produtos' ? 'bg-brand-50 text-brand-700 tracking-wide translate-x-1 shadow-sm' : 'text-gray-600 hover:bg-gray-50 hover:text-brand-600'"
            @click="menuOpen = false">PRODUTOS</NuxtLink>
          ...
          <NuxtLink to="/info"
            class="relative z-10 block py-3 px-6 font-semibold rounded-2xl transition-all duration-300 active:scale-[0.98] hover:translate-x-1 origin-left"
            :class="activeSection === 'info' ? 'bg-brand-50 text-brand-700 tracking-wide translate-x-1 shadow-sm' : 'text-gray-600 hover:bg-gray-50 hover:text-brand-600'"
